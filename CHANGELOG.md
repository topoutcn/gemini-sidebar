...
## 2026-01-31 v1.0.1 — 翻译按钮拆分 + 视频字幕翻译

### 变更

- **去掉「总结」按钮**，第一排按钮改为：分析此页 | 翻译 | 译视频
- **新增「译视频」按钮** — YouTube 专用，自动点击页面「内容转文字」展开转录面板，读取完整字幕发给 Gemini 翻译
  - 非 YouTube 页面点击会 toast 提示仅支持 YouTube
  - 无字幕视频会 toast 提示失败并建议用「分析此页」
  - 错误提示显示 7 秒（普通提示 5 秒）
- **`background.js`** 新增 `getActiveTab` 消息处理，供侧边栏判断当前页面类型
- **`youtube-content-script.js`** 新增 `openAndReadTranscript()` 函数：展开描述区 → 点击「内容转文字」→ 轮询读取字幕
